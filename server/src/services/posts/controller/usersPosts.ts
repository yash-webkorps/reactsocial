...
const usersPosts = async (req: Request, res: Response) => {
    try {
        ...
        let order: OrderItem[] = [["createdAt", "DESC"]]; // newest
        ...
        const postWithComments = await Promise.all(postsWithLikeStatusAndUsers.map(async (post) => {
